...
// MARK: - Git Manager
@MainActor  // Добавляем аннотацию для выполнения всех операций на главном потоке
class GitManager: ObservableObject {
    ...
    func processRepositories(totalCommits: Int?) {
        ...
        processingTask = Task { [weak self] in
            guard let self = self else { return }  // Используем слабую ссылку на self
            
            let availableRepos = self.repositories.filter { $0.isEnabled && self.checkAccess(path: $0.path) }
            guard !availableRepos.isEmpty else {
                await MainActor.run {  // Выполняем обновление UI на главном потоке
    ...
    public func addLog(_ message: String) {
        ...
        logs.append(newLog)  // Без DispatchQueue.main, так как мы на @MainActor
    }
                    ...
                    ForEach(gitManager.repositories) { repo in
                        HStack {
                            Toggle(isOn: Binding(
                                ...
                                set: { _ in
                                    Task { @MainActor in
                                        gitManager.toggleRepository(repo)
                                    }
                                }
                            )) {
                                Text(repo.path)
                            ...
                            Button(action: {
                                Task { @MainActor in
                                    gitManager.removeRepository(repo)
                                }
                            }) {
